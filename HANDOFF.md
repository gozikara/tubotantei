...
- 最新コミット: `146f08c`「クイズ内容をadmin.htmlのエピソード情報に沿って作り直し」
  (`ceb86e1` バトル画面のポケモン対戦画面化 → `81db800` 初回実装)
- これらのコミットは**まだリモートに無い**。下記の bundle か zip から取り込むこと。
- クイズは admin.html のエピソード情報(原因筋・経穴)から作成し、紛らわしい本物の選択肢
  ＋症例ベースの問題文に。選択肢はゲーム側でシャッフル表示される。各問に hint 付き。
- バトル画面は本家ポケモンの対戦画面レイアウト(敵/味方ステータス枠・戦闘台・青枠メッセージ・
  2×2コマンド・技選択・こうかばつぐん/反撃/ひんし/けいけんち獲得)を再現済み。
